...
> **2026-09-15 — фронтенд-часть плана заменена.** Встраивание SSO-кнопок в платформенный `LoginPage`
> через слот `auth:login-page:additional` (§4.2, §5, шаг 12) отменено: страница логина вынесена из
> платформы, а метод аутентификации стал свойством узла. SSO-модуль должен регистрировать свой метод
> в `auth-node` (`registerMethod` с `loginUrl` на свой вход) — см.
> [`plans/2026-09-15-auth-node-method.md`](2026-09-15-auth-node-method.md). Бэкенд-часть (§1–3, §6–7)
> остаётся в силе; `SsoRegistry` из §3.2 при реализации сводится к регистрации методов.
