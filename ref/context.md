...
| `support-chat`, `admin:apps`, `floating` | первое встроенное приложение админки: обращения + SSE через Redis-канал, несколько обращений на пользователя, портальная страница «Мои обращения» (`/support`), плавающий виджет-карточка на точке `floating`, реестр приложений, транскрипты на `MessageScroller` с группировкой, вложения через сервис `storage`; статусы open/pending/solved/closed с закрытием пользователем, инциденты (серьёзность, привязка дублей, рассылка); задел под AI — роль `ai` и сервис `support-chat`; направление AI-first (ассистент по данным платформы, эскалация с брифом, инциденты, пользовательское закрытие) — `plans/2026-09-18-support-chat-ai-first.md` — packages/module-support-chat, packages/module-admin/docs/reference/composition.md |
| `module-metrics`, `metrics:*`, `secrets` | журнал событий (этап 00, `packages/module-metrics`): партиционированные `metrics.events` + синглтон настроек, `POST /api/metrics/collect` с uuid-дедупом и лимитами, HMAC-псевдонимизация через core-сервис `secrets` (`AMPLICADA_METRICS_PSEUDONYM_SALT`), трекер `page.view` на `floating`, админ-приложение `/admin/apps/metrics`, минутный rate-limit приёма (429), opt-out/DNT, клики `data-metrics`, каталог событий, технические метрики роутов (RED, p50/p95/p99) и SQL (fingerprint, slow samples) через core-точки `http:observer`/`request-context`, бизнес-метрики модулей (`metrics.emit`, `metrics:definitions`, панели `metrics:panels`; пилот support-chat), группировка клиентских ошибок в issues (fingerprint: тип + маршрут + кадры) и Web Vitals (p75/p95 из гистограмм), выходы (webhook с HMAC, durable outbox с ретраями/DLQ, журнал доставки, CSV-экспорт; выключены по умолчанию), алерты (пороги/absence по событиям и измерениям, pending→firing→resolved, шина `metrics.alert`), guardrail кардинальности (`__overflow`) и панель «Здоровье» (`GET /admin/health`), задача `metrics.maintenance` (партиции/retention); реестр долгов и отложенного — `plans/2026-09-18-metrics-module/99-gaps-and-deferred.md`; направление целиком — `plans/2026-09-18-metrics-module/` (измерения/HTTP/SQL/RUM, `metrics:definitions`, sinks Яндекс.Метрика/webhook, алерты; ещё нужны core-точки `http:observer` и хук ошибок React 19) |
