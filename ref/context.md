...
| `module-metrics`, `metrics:*`, `http:observer`, `MetricSink` | направление метрик (`plans/2026-09-18-metrics-module/`): свой оффлайн-стек в Postgres (события + измерения, роллапы, retention), контракты шире реализации — источники/коллекторы, `MetricsService.emit`, store с query-AST, `metrics:definitions`/`metrics:panels`, sinks (Яндекс.Метрика/ webhook/ CSV, будущие OTLP/PRW), алерты через task-scheduler и event bus; псевдонимизация HMAC, внешние выходы выключены по умолчанию; нужны точки в core: `http:observer` (root `onResponse`), `secrets`/`config`, хук ошибок React 19 |
