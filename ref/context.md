...
| `module-metrics`, `metrics:*`, `secrets` | журнал событий (этап 00, `packages/module-metrics`): партиционированные `metrics.events` + синглтон настроек, `POST /api/metrics/collect` с uuid-дедупом и лимитами, HMAC-псевдонимизация через core-сервис `secrets` (`AMPLICADA_METRICS_PSEUDONYM_SALT`), трекер `page.view` на `floating`, админ-приложение `/admin/apps/metrics`, минутный rate-limit приёма (429), opt-out/DNT, клики `data-metrics`, каталог событий, задача `metrics.maintenance` (партиции/retention); направление целиком — `plans/2026-09-18-metrics-module/` (измерения/HTTP/SQL/RUM, `metrics:definitions`, sinks Яндекс.Метрика/webhook, алерты; ещё нужны core-точки `http:observer` и хук ошибок React 19) |
