...
| `notes/module-metrics.md` | `in-progress` | D-001…D-017: события — партиционированные таблицы, uuid-идемпотентность, HMAC-псевдонимизация (core `secrets`), единая точка записи, retention через `DETACH`, same-origin ингестия, polling; технические метрики, бизнес-события и панели, ошибки и Web Vitals, выходы (webhook/outbox), алерты, guardrails и health |
...
| `module-metrics` | [`packages/module-metrics/docs/`](../packages/module-metrics/docs/index.md) — события, HTTP API, хранение и настройки, трекер |
...
| `plans/2026-09-18-metrics-module/` | `in-progress` | `module-metrics` — свой закрытый оффлайн-стек метрик: продуктовые события, бизнес-метрики модулей и технические (роуты/SQL/ошибки) в Postgres с роллапами и retention; контракты шире реализации (источники, события/измерения, store, query-AST, определения/панели, sinks, алерты), внешние выходы (Яндекс.Метрика, webhook, CSV, будущие OTLP/PRW) — адаптеры, выключенные по умолчанию; псевдонимизация и 152-ФЗ. `00-overview` (рамка и этапы 00–06), `01-contracts` (каталог контрактов), `02-offline-stack` (схема, конвейеры, алерты, sinks, масштаб, документация), `99-gaps-and-deferred` (реестр долгов, матрица проверок, следующие работы) |
