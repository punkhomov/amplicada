...
Что входит: API обращений и сообщений, статусная модель «чей ход»
(open/pending/solved/closed) с закрытием со стороны пользователя, инциденты
(серьёзность, привязка дублей, рассылка обновлений), почтовые папки и настройки в
админке (задел под AI-провайдера), SSE-доставка изменений, вложения (S3-хранилище core,
картинки превью), плавающий виджет пользователя, портальная страница «Мои обращения»
(список обращений и история переписки), админ-приложение (список обращений, переписка,
ответ, статусы, инциденты), транскрипты на вендоренном `MessageScroller`.
...
| Папки и настройки поддержки | `lib/folders.ts`, `widgets/support-settings-dialog`, `support_chat.settings` | `src/frontend/pages/support-chat-admin`, `migrations/0005_settings.sql` |
...
| Статусы и жизненный цикл обращений | [reference/api.md](./reference/api.md) |
| Инциденты (серьёзность, привязка, рассылка) | [reference/api.md](./reference/api.md) |
