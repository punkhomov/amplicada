...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      from: {
        ...
        pathNot: [
          ...
          '^apps/[^/]+/src/(?:index|main)[.](?:ts|tsx)$', // app entry points (started, not imported)
          '^packages/platform-core/src/frontend/ui/', // vendored shadcn/ui kit — installed in bulk, consumed on demand
          '^packages/[^/]+/src/testing/', // фикстуры для тестов: тесты из обхода исключены, поэтому потребителей у них не видно
        ],
      },
      to: {},
    },
    ...
    // ── Backend ↔ Frontend ────────────────────────────────────────────
    {
      ...
      severity: 'error',
      comment: 'Backend не импортирует frontend. Используй contracts для общих типов.',
      from: { path: '/backend/' },
      to: { path: '/frontend/' },
    },
    {
      ...
      severity: 'error',
      comment: 'Frontend не импортирует backend. Используй contracts для общих типов.',
      from: { path: '/frontend/' },
      to: { path: '/backend/' },
    },
    ...
    // ── Contracts ─────────────────────────────────────────────────────
    {
      ...
      severity: 'error',
      comment: 'contracts не должен импортировать runtime-код — только типы.',
      from: { path: '/contracts/' },
      to: {
    ...
    {
      name: 'contracts-no-heavy-deps',
      severity: 'warn',
      comment: 'contracts не должен импортировать React, Fastify, Drizzle и т.д.',
      from: { path: '/contracts/' },
      to: {
        dependencyTypes: ['npm'],
        path: '^(react|drizzle-orm|fastify|@fastify|pino)',
      },
    },

    // ── Cross-module ──────────────────────────────────────────────────
    {
      name: 'module-no-cross-backend',
      severity: 'error',
      comment: 'Модули импортируют backend друг друга только из platform-core. Остальное — через contracts.',
      from: {
        path: '^packages/(?!platform-core)/',
      },
      to: {
        path: '^packages/(?!(platform-core|module-workflow)/).*/backend/',
      },
    },
    {
      name: 'module-no-cross-frontend',
      severity: 'error',
      comment: 'Модули не импортируют frontend друг друга — только из platform-core.',
      from: {
        path: '^packages/(?!platform-core)/.*/frontend/',
      },
      to: {
        path: '^packages/(?!platform-core)/.*/frontend/',
      },
    ...
    {
      name: 'module-no-cross-module-runtime',
      severity: 'error',
      comment: 'Модули импортируют runtime друг друга только через platform-core. Для типов — contracts.',
      from: {
        path: '^packages/(?!platform-core)/',
      },
      to: {
    ...
    {
      name: 'app-api-no-frontend',
      severity: 'error',
      comment: 'apps/api — только backend. Не импортирует frontend.',
      from: { path: '^apps/api/' },
      to: { path: '/frontend/' },
    },
    {
      name: 'app-web-no-backend',
      severity: 'error',
      comment: 'apps/web — только frontend. Не импортирует backend (кроме contracts).',
      from: { path: '^apps/web/' },
      to: { path: '/backend/', pathNot: '/contracts/' },
    },

    // ── Backend layering (слабая) ─────────────────────────────────────
    {
      name: 'backend-documents-no-routes',
      severity: 'warn',
      comment: 'documents не импортирует routes.',
      from: { path: '/backend/documents/' },
      to: { path: '/backend/routes/' },
    },
    {
      name: 'backend-services-no-routes',
      severity: 'warn',
      comment: 'services не импортирует routes напрямую.',
  ...
  options: {
    ...
    exclude: {
      path: '(node_modules|dist|\\.test\\.|__tests__|migrations/)',
    },
...
// generated: dependency-cruiser@18.1.0 on 2026-07-22T11:58:43.130Z
